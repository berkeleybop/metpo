...
def main(sssom_file, distance_threshold):
    ...
    click.echo(f"{'Ontology':<15} {'Matches':<10} {'Best':<10} {'Median':<10} {'Worst':<10} {'Avg':<10} {'<{distance_threshold}':<8} {'Assessment'}")
    click.echo("-" * 110)
    ...
    for ont in sorted(non_ols):
        ...
        if len(ont_df) == 0:
            click.echo(f"{ont:<15} {'0':<10} {'-':<10} {'-':<10} {'-':<10} {'-':<10} {'0':<8} REMOVE - no matches")
            continue
        ...
        click.echo(f"{ont:<15} {len(ont_df):<10} {best:<10.4f} {median:<10.4f} {worst:<10.4f} {avg:<10.4f} {high_quality:<8} {assessment}")

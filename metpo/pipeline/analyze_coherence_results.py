...
def main(results_file: str, mappings_file: str):
    ...
    if len(high_coherence) > 0:
        ...
        for _idx, row in high_coherence.iterrows():
            ...
            print(
                f"  Coherence: {row['coherence_score']:.1%} ({row['coherent_sibling_count']}/{row['metpo_sibling_count']} siblings align)"
            )
            ...
            if len(all_matches) > 1:
                ...
                for _, match in all_matches.head(5).iterrows():
                    print(
                        f"    {match['match_document'][:50]:50} ({match['match_ontology']:8}) dist={match['distance']:.3f}"
                    )
    else:
    ...
    moderate = meaningful[
        (meaningful["coherence_score"] >= 0.3) & (meaningful["coherence_score"] < 0.5)
    ...
    if len(moderate) > 0:
        ...
        for _idx, row in moderate.head(10).iterrows():
            print(
                f"{row['metpo_label']:30} → {row['match_ontology']:10} "
    ...
    ontology_stats = (
        meaningful.groupby("match_ontology")
        .agg({"coherence_score": ["mean", "median", "count"], "match_distance": ["mean", "median"]})
        .round(3)
    )
    ...
    meaningful["alignment_score"] = meaningful["coherence_score"] * (
        1 - meaningful["match_distance"]
    )
    best_candidates = meaningful.nlargest(20, "alignment_score")

    print("\nTop 20 candidates for importing/aligning with external ontologies:\n")
    print(
        f"{'Rank':<5} {'METPO Term':<35} {'Ontology':<10} {'Coherence':<12} {'Distance':<10} {'Score':<8}"
    )
    print("-" * 90)

    for rank, (_idx, row) in enumerate(best_candidates.iterrows(), 1):
        print(
            f"{rank:<5} {row['metpo_label'][:33]:<35} {row['match_ontology']:<10} "

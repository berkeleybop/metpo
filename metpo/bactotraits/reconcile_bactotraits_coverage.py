...
def get_bactotraits_field_values(
    field_path: str, db_name: str = "bactotraits", collection_name: str = "bactotraits"
) -> dict[str, str]:
    """
...
def get_all_bactotraits_fields(
    db_name: str = "bactotraits", collection_name: str = "bactotraits"
) -> list[str]:
    """
...
def get_all_bactotraits_values_with_fields(
    db_name: str = "bactotraits", collection_name: str = "bactotraits"
) -> dict[str, list[str]]:
    """
...
def reconcile_all_field_names(
    ...
    for sanitized_field in sorted(sanitized_fields):
        ...
        for variant in set(field_variants):  # Use set to avoid redundant checks
            if variant in bactotraits_synonyms:
                matched, matched_info, matched_variant, matched_source = (
                    True,
                    bactotraits_synonyms[variant],
                    variant,
                    "bactotraits",
        ...
        if not matched:
            for variant in set(field_variants):
                if variant in all_metpo_synonyms:
                    matched, matched_info, matched_variant, matched_source = (
                        True,
                        all_metpo_synonyms[variant],
                        variant,
                        all_metpo_synonyms[variant].get("source", "unknown"),
                    )
                    break
        ...
        if matched:
            ...
            entry = {
                ...
                "original_kgmicrobe_name": original_field
                if original_field != sanitized_field
                else None,
    ...
    missing_permanently_excluded = [
        e for e in missing_entries if e["field"] in PERMANENTLY_EXCLUDED_FIELDS
    ]
    missing_identifiers = [e for e in missing_entries if e["field"] in IDENTIFIER_FIELDS]
    missing_traits = [
        e
        for e in missing_entries
        if e["field"] not in PERMANENTLY_EXCLUDED_FIELDS and e["field"] not in IDENTIFIER_FIELDS
    ]
...
def reconcile_coverage(
    ...
    if output_format == "yaml":
        ...
        for entry in covered_entries:
            ...
            if entry["label"]:
                print(
                    f"  ✓ '{entry['value']}' → {entry['metpo_id']} ({entry['label']}){normalization_note}{matched_note}"
                )
            else:
                print(
                    f"  ✓ '{entry['value']}' → {entry['metpo_id']}{normalization_note}{matched_note}"
                )

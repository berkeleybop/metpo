...
def extract_traits(html: str) -> list[dict]:
    """Extract trait card data from the metaTraits /traits HTML page."""
    ...
    for i, start in enumerate(card_starts):
        ...
        onto_refs = re.findall(
            r'class="btn btn-cta" href=([^>]+)>([^<]+)</a>', section
        )
        ...
        traits.append(
            {
                ...
                "type": badge_val,
                "ontology_refs": [
                    {"curie": r[1].strip(), "url": r[0].strip()} for r in onto_refs
                ],

...
def parse_metpo_template(path: str) -> tuple:
    ...
    with open(path, newline="") as f:
        ...
        header = next(reader)  # human-readable header
        ...
        for row in reader:
            ...
            for source in def_sources.split("|"):
                source = source.strip()
                if re.match(r"^[A-Za-z][A-Za-z0-9_]+:\S+$", source) and "://" not in source:
                    curie_to_metpo[source].append({"id": metpo_id, "label": label})

    return curie_to_metpo, metpo_labels


def parse_metpo_properties(path: str) -> dict:
    ...
    """
    category_to_props = defaultdict(lambda: {"positive": None, "negative": None})
    all_props = {}
...
def join_composed_traits(cards, curie_to_metpo):
    ...
    for card in composed_cards:
        ...
        substrate_curies_str = "; ".join(sorted(card["substrate_curies"])) if card["substrate_curies"] else ""
        matched = False
            ...
            if curie in curie_to_metpo:
                for metpo in curie_to_metpo[curie]:
                    mappings.append({
                        ...
                        "subject_source": "metatraits",
                        "object_source": "METPO",
                        "comment": f"Process CURIE: {curie} | Substrate: {card['substrate']} ({substrate_curies_str})",
                    })
                    matched = True
        ...
        if not matched and base_norm in base_category_metpo:
            for metpo in base_category_metpo[base_norm]:
                mappings.append({
                    ...
                    "subject_source": "metatraits",
                    "object_source": "METPO",
                    "comment": f"Inherited from base '{card['base_category']}' | Substrate: {card['substrate']} ({substrate_curies_str})",
                })
                matched = True
...
def resolve_kgx_edges(cards, category_to_props):
    ...
    for card in composed_cards:
        ...
        edges.append({
            ...
            "negative_predicate_label": props["negative"]["label"] if props.get("negative") else "",
...
def write_sssom(mappings, output_path):
    ...
    with open(outpath, "w", newline="") as f:
        ...
        f.write("# comment: CURIE-based join (no fuzzy matching). Composed traits use two-part model: process (METPO) + substrate (CHEBI).\n")
        f.write("#\n")
...
def write_kgx_edges(edges, output_path):
    ...
    with open(outpath, "w", newline="") as f:
        ...
        f.write("#   e.g.: NCBITaxon:562 --[METPO:2000037 does not ferment]--> CHEBI:17057 (cellobiose)\n")
        f.write("#\n")
...
def write_report(
    mappings, base_mappings, base_unmatched,
    composed_mappings, composed_unmatched,
    cards, curie_to_metpo, category_to_props,
    kgx_edges, resolved_categories, unresolved_categories,
    output_path,
):
    ...
    lines = []
    lines.append(f"# MetaTraits-to-METPO CURIE Join Report\n")
    lines.append(f"**Date:** {today}\n")
    lines.append("**Method:** Deterministic CURIE set intersection + METPO property synonym resolution\n\n")
    ...
    lines.append(f"| METPO CURIE cross-references | {sum(len(v) for v in curie_to_metpo.values())} |\n")
    ...
    mapped_subjects = set(m["subject_id"] for m in mappings)
    mapped_objects = set(m["object_id"] for m in mappings)
    ...
    lines.append(f"| Composed traits unmatched | {len(composed_unmatched)} / {len(composed_cards)} |\n")
    ...
    lines.append("METPO object properties resolved from MetaTraits base categories via synonym matching.\n\n")
    lines.append("| Metric | Count |\n|--------|------:|\n")
    ...
    lines.append("| MetaTraits Category | METPO Predicate (+) | METPO Predicate (-) | Composed Traits |\n")
    lines.append("|--------------------|--------------------|--------------------|---------:|\n")
    ...
    for m in sorted(base_mappings, key=lambda x: x["subject_label"]):
        curie_part = m["comment"].replace("Shared CURIE: ", "").replace("Generic CURIE fallback: ", "")
        lines.append(f"| {m['subject_label']} | {m['object_label']} | `{curie_part}` | {m['confidence']} |\n")
    lines.append("\n")
    ...
    if fully_unmatched:
        ...
        lines.append("Many of these **do** resolve to METPO properties (see KGX section above).\n\n")
    ...
    lines.append("- Coverage gaps in SSSOM indicate where embeddings or manual curation would add value.\n")
    lines.append("- The two approaches are complementary: CURIE join for grounded SSSOM mappings,\n")
...
def main(metatraits_cards, metpo_template, metpo_properties, output, kgx_edges, report, include_generic):
    ...
    if category_to_props:
        ...
        click.echo(f"  {len(resolved_categories)} categories resolved, {len(unresolved_categories)} unresolved")
    ...
    if report:
        write_report(
            mappings, base_mappings_raw, base_unmatched_all,
            composed_mappings_raw, composed_unmatched,
            cards, curie_to_metpo, category_to_props,
            kgx_edge_list, resolved_categories, unresolved_categories,
            report,
        )

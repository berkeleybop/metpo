...
def get_madin_field_values(
    field_path: str, db_name: str = "madin", collection_name: str = "madin"
) -> dict[str, str]:
    """
...
def get_all_madin_values_with_fields(
    db_name: str = "madin", collection_name: str = "madin"
) -> dict[str, list[str]]:
    """
...
def reconcile_all_field_names(
    tsv_path: str, output_format: str = "text", output_file: str | None = None
) -> None:
    """
    ...
    for field_path in sorted(madin_fields):
        ...
        if field_path_normalized in madin_synonyms or field_name in madin_synonyms:
            ...
            missing_entries.append({"field": field_path, "unique_values": value_count})
    ...
    if output_format == "yaml":
        ...
        for entry in covered_entries:
            if entry["label"]:
                print(
                    f"  ✓ '{entry['field']}' → {entry['metpo_id']} ({entry['label']}) [{entry['unique_values']} unique values]"
                )
            else:
                print(
                    f"  ✓ '{entry['field']}' → {entry['metpo_id']} [{entry['unique_values']} unique values]"
                )
...
def reconcile_coverage(
    field_path: str, tsv_path: str, output_format: str = "text", output_file: str | None = None
) -> None:
    """
    ...
    if output_format == "yaml":
        ...
        for entry in covered_entries:
            normalization_note = (
                f" [NORMALIZED from '{entry['original_value']}']" if entry["original_value"] else ""
            )
            if entry["label"]:
                print(
                    f"  ✓ '{entry['value']}' → {entry['metpo_id']} ({entry['label']}){normalization_note}"
                )
            else:
        ...
        if missing_entries:
            ...
            for entry in missing_entries:
                normalization_note = (
                    f" [NORMALIZED from '{entry['original_value']}']"
...
def verify_madin_synonyms(
    ...
    if output_format == "yaml":
        ...
        for entry in sorted(verified_entries, key=lambda x: x["synonym"]):
            fields_str = (
                ", ".join(entry["found_in_fields"])
                if entry["field_count"] <= 3
                else f"{', '.join(entry['found_in_fields'][:3])}, ..."
            )
            if entry["label"]:
                print(
                    f"  ✓ '{entry['synonym']}' ({entry['metpo_id']} [{entry['entity_type']}] - {entry['label']}) in {entry['field_count']} field(s): [{fields_str}]"
                )
            else:
                print(
                    f"  ✓ '{entry['synonym']}' ({entry['metpo_id']} [{entry['entity_type']}]) in {entry['field_count']} field(s): [{fields_str}]"
                )
        ...
        if unverified_entries:
            ...
            print("-" * 80)
            print(
                "These synonyms are attributed to Madin in METPO but NOT found in Madin field VALUES:"
            )
            ...
            for entry in sorted(unverified_entries, key=lambda x: x["synonym"]):
                if entry["label"]:
                    print(
                        f"  ✗ '{entry['synonym']}' ({entry['metpo_id']} [{entry['entity_type']}] - {entry['label']})"
                    )
                else:
                    print(
                        f"  ✗ '{entry['synonym']}' ({entry['metpo_id']} [{entry['entity_type']}])"
                    )
...
def generate_integrated_report(
    tsv_path: str, output_format: str = "yaml", output_file: str | None = None
) -> None:
    """
    ...
    for field_name in sorted(madin_fields):
        ...
        if field_name == "pathways":
                    ...
                    covered_pathways.append(
                        {
                            ...
                            "entity_type": ENTITY_TYPE_MAP.get(
                                info.get("entity_type", "unknown"), "unknown"
                            ),
                        }
                    )
                else:
            ...
            value_coverage_pct = (
                100 * len(covered_pathways) / total_values if total_values > 0 else 0
            )
        else:
        ...
        if field_mapped:
            ...
            field_entry = {
                ...
                if len(covered_values) <= 20
                else covered_values[:20],
                "missing_values": missing_values
                if len(missing_values) <= 20
                else missing_values[:20],
    ...
    if output_format == "yaml":
        result = {
            "madin_metpo_reconciliation": {
                "summary": {
                    ...
                    "fields_with_full_value_coverage": sum(
                        1 for f in field_analysis if f.get("value_coverage_percentage") == 100
                    ),
        ...
        print(
            f"  Fields with name mapping: {fields_mapped} ({100 * fields_mapped / total_fields:.1f}%)"
        )
        print(
            f"  Fields with 100% value coverage: {fields_full_coverage} ({100 * fields_full_coverage / total_fields:.1f}%)"
        )
        print(f"  Total METPO Madin synonyms: {len(madin_synonyms)}")
        print(
            f"  Verified synonyms: {len(verified_synonyms)} ({100 * len(verified_synonyms) / len(madin_synonyms):.1f}%)"
        )
        print(
            f"  False synonym claims: {len(false_claims)} ({100 * len(false_claims) / len(madin_synonyms):.1f}%)\n"
        )
        ...
        for field in field_analysis:
            ...
            if field["field_mapped"]:
                ...
                print(
                    f"  → {field['field_metpo_id']} ({field['field_metpo_label']}) [{field['field_entity_type']}]"
                )
                print(
                    f"  Values: {field['covered_values_count']}/{field['total_values']} covered ({coverage})"
                )
                ...
                if field["missing_values"] and len(field["missing_values"]) <= 10:
                    print(
                        f"  Missing values: {', '.join(str(v) for v in field['missing_values'][:5])}"
                    )
            else:
        ...
        if false_claims:
            ...
            for claim in false_claims:
                print(
                    f"  ✗ '{claim['synonym']}' ({claim['metpo_id']} [{claim['entity_type']}] - {claim['label']})"
                )
...
@click.option(
    "--mode",
    type=click.Choice(
        ["values", "field_names", "verify_synonyms", "integrated"], case_sensitive=False
    ),
    default="values",
